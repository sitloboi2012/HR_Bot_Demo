...
class BaseDocumentLoader(ABC):
    def __init__(self, vector_db: Any, embedding_model: Any, text_splitter: Any):
...
class PDFDocumentLoader(BaseDocumentLoader):
    ...
    def __init__(self, vector_db: Any, embedding_model: Any, text_splitter: Any):
        super().__init__(vector_db, embedding_model, text_splitter)
    ...
    def sort_vertical(self, input_dicts: Dict[str, str | float | int]) -> List[str]:
        ...
        while remain:
            ...
            for i, value in enumerate(remain):
                ...
                diff_top_left_x, diff_top_left_y = abs(current_top_left[0] - value["top_left"][0]), abs(current_top_left[1] - value["top_left"][1])
                diff_bottom_left_x, diff_bottom_left_y = abs(current_bottom_left[0] - value["bottom_left"][0]), abs(current_bottom_left[1] - value["bottom_left"][1])

                if (line_height_diff[0] <= 5 and line_height_diff[1] <= 30) and (diff_top_left_x <= 5 and diff_top_left_y <= 20) and (diff_bottom_left_x <= 5 and diff_bottom_left_y <= 20):
                    current_text += f" \n {value['text']}"
    ...
    def upload_to_db(self, document: LangChainDocument):
